...
if __name__ == "__main__":
    ...
    URL_TEMPLATE = "https://mi.api/columns/{dataset_id}"  # opcional si necesitas columnas "oficiales"

    try:
        # Ejemplo 1: solo DataFrame y columnas del DataFrame (sin endpoint)
        r1 = get_df_unificado(
            dataset_id=DATASET_ID,
            fecha_inicio=FECHA_INICIO,
            fecha_final=FECHA_FINAL,
        ...
        print(f"[Ejemplo 1] Filas DF: {len(r1['dataframe'])}, Columnas: {r1['namecolumns']}")

        # Ejemplo 2: con endpoint de columnas (y fallback automático a df.columns si falla)
        r2 = get_df_unificado(
            dataset_id=DATASET_ID,
            fecha_inicio=FECHA_INICIO,
            fecha_final=FECHA_FINAL,
            url_template_namecols=URL_TEMPLATE,
            include_namecolumns=True,
            timeout_namecols=20.0,
            retries_namecols=3,
        )
        print(f"[Ejemplo 2] Filas DF: {len(r2['dataframe'])}, Columnas: {r2['namecolumns']}")

        # Ejemplo 3: usar directamente la facade para columnas
        # cols = obtener_namecolumns(DATASET_ID, URL_TEMPLATE)
        # print("[Ejemplo 3] nameColumns:", cols)

    except ValidationError as ve:
        log.error("Error de validación: %s", ve)
    except ExternalServiceError as se:
        log.error("Error de servicio externo: %s", se)
    except Exception as e:
        log.exception("Error inesperado: %r", e)